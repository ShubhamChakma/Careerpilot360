...
export default function CareerPathStepper({ steps, isDark }) {
  return (
    <div>
      <p className="text-[10px] font-bold text-[#6B6B6B] mb-1.5 uppercase tracking-wider">Recommended Learning Path</p>
      <div className="flex items-center gap-1.5 flex-wrap">
        {steps.map((step, i) => (
          <React.Fragment key={step}>
            ...
            {i < steps.length - 1 && <span className="text-[#5A5A5A] text-[10px]">→</span>}
          </React.Fragment>
        ))}
      </div>
    </div>
